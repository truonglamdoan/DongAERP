...
namespace DongAERP.Areas.Admin.Controllers
{
    public class ReportForMaketsController : Controller
    {
        ...
        /// <param name="toDay"></param>
        /// <returns></returns>
        public ActionResult SearchReportMaketForYear([DataSourceRequest]DataSourceRequest request, DateTime fromYear, DateTime toYear, string reportTypeID)
        {
            List<ReportForMaket> listData = new ReportBL().SearchReportMaketForYear(fromYear, toYear, reportTypeID);
            foreach (ReportForMaket item in listData)
            {
                double sumTong = item.American + item.Asia + item.Global + item.Europe + item.Canada + item.Australia;
        ...
        /// </history>
        [HttpPost]
        public ActionResult SearchLineChartMaketReportForYear([DataSourceRequest]DataSourceRequest request, DateTime fromYear, DateTime toYear, string reportTypeID)
        {
            List<ReportForMaket> listData = new ReportBL().SearchReportMaketForYear(fromYear, toYear, reportTypeID);
            foreach (ReportForMaket item in listData)
            {
        ...
        {
            List<ReportForMaket> listData = new ReportBL().DataReportMaketForGradationCompare(year, int.Parse(gradation), reportTypeID);
