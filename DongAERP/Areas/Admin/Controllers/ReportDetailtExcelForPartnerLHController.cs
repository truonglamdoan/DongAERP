...
namespace DongAERP.Areas.Admin.Controllers
{
    public class ReportDetailtExcelForPartnerLHController : Controller
    {
        ...
        public ActionResult CreateExcelForDayMonthYear(DateTime fromDate, DateTime toDate, int typeID, string reportTypeID)
        {
            ...
            string templatePath = "~/Content/Report/ReportDetailtByPartnerLH.xlsx";
            // Get đường dẫn
            ...
            switch (typeID)
            {
                // Theo ngày
                case 1:
                    listReportData = new ReportBL().SearchReportDetailtPartnerForDay(fromDate, toDate, reportTypeID);
                ...
                case 2:
                    ...
                    listReportData = new ReportBL().SearchPartnerForTotalForMonth(fromDate, toDate, reportTypeID);
                ...
                default:
                    ...
                    listReportData = new ReportBL().SearchPartnerForTotalForYear(fromDate, toDate, reportTypeID);
        ...
        public ActionResult CreateExcelForDayMonthYearForOne(DateTime fromDate, DateTime toDate, int typeID, string reportTypeID, string partnerID)
        {
            ...
            switch (typeID)
            {
                // Theo ngày
                case 1:
                    listReportData = new ReportBL().SearchPartnerForOne(fromDate, toDate, reportTypeID, partnerID);
                    foreach (ReportDetailtForPartner item in listReportData)
                    {
                        item.STT = "Ngày " + item.CreatedDate.ToString("dd/MM/yyyy");
                        item.ReportID = item.PartnerName;
                        item.TongDS = item.DSChiNha + item.DSChiQuay + item.DSCK;
                    }
                    ...
                    if (listReportData.Count > 0)
                    {
                        sheetReport.Cells["D3"].PutValue("Đối tác");
                        sheetReport.Cells["E3"].PutValue(listReportData[0].PartnerName);
                        namePartner = listReportData[0].PartnerName;
                    }
                    sheetReport.Cells["E4"].PutValue(fromDate.ToString("dd/MM/yyyy"));
                    sheetReport.Cells["G4"].PutValue(toDate.ToString("dd/MM/yyyy"));
                    break;
                ...
                case 2:
                    ...
                    listReportData = new ReportBL().SearchPartnerForOneForMonth(fromDate, toDate, reportTypeID, partnerID);
                    foreach (ReportDetailtForPartner item in listReportData)
                    {
                ...
                default:
                    ...
                    listReportData = new ReportBL().SearchPartnerForOneForYear(fromDate, toDate, reportTypeID, partnerID);
        ...
        {
            WorkbookDesigner designer = new WorkbookDesigner();
            string templatePath = "~/Content/Report/ReportDetailtByPartnerLHForGradationCompare.xlsx";
            // Get đường dẫn
            templatePath = System.Web.HttpContext.Current.Server.MapPath(templatePath);
            ...
            List<ReportDetailtForPartner> listDataGradation = new ReportBL().ReportDetailtPartnerGradationCompareForAll(year, int.Parse(gradationID), reportTypeID);
            ...
            if (listDataGradation.Count() > 0)
            {
                ...
                foreach (ReportDetailtForPartner item in listDataGradation)
                {
                    ...
                    // Cùng kì
                    ReportDetailtForPartner dataItemLastYear = listDataGradation.Find(x => x.PartnerName == item.PartnerName && x.Year == (year - 1).ToString());
                    ReportDetailtForPartner dataItemYear = listDataGradation.Find(x => x.PartnerName == item.PartnerName && x.Year == year.ToString());
        ...
        public ActionResult CreateExcelGradationCompareForOne(string gradationID, int year, string reportTypeID, string partnerID)
        {
            ...
            sheetReport.Cells["D4"].PutValue("");
            sheetReport.Cells["F4"].PutValue("");
            ...
            List<ReportDetailtForPartner> listReportData = new ReportBL().ReportDetailtPartnerGradationCompareForOne(year, int.Parse(gradationID), reportTypeID, partnerID);
            if(listReportData.Count > 0)
            {
                namePartner = listReportData[0].PartnerName;
            }
            ...
            {
                sumDSChiQuay = listReportData[1].DSChiQuay - listReportData[0].DSChiQuay;
                sumDSChiNha = listReportData[1].DSChiNha - listReportData[0].DSChiNha;
                sumDSCK = listReportData[1].DSCK - listReportData[0].DSCK;
                sumTongDS = listReportData[1].TongDS - listReportData[0].TongDS;
                ...
                listReportData.Add(
                    ...
                    {
                        PartnerName = string.Format("Tăng giảm so với cùng kì {0} (%)", year - 1),
                        DSChiQuay = listReportData[0].DSChiQuay == 0 ? 0 : Math.Round((sumDSChiQuay / listReportData[0].DSChiQuay) * 100, 2, MidpointRounding.ToEven),
                        DSChiNha = listReportData[0].DSChiNha == 0 ? 0 : Math.Round((sumDSChiNha / listReportData[0].DSChiNha) * 100, 2, MidpointRounding.ToEven),
                        DSCK = listReportData[0].DSCK == 0 ? 0 : Math.Round((sumDSCK / listReportData[0].DSCK) * 100, 2, MidpointRounding.ToEven),
                        TongDS = listReportData[0].TongDS == 0 ? 0 : Math.Round((sumTongDS / listReportData[0].TongDS) * 100, 2, MidpointRounding.ToEven),
                    }
                );
            }
            ...
            //Add Pie Chart
            // Chi Nhà
            int chartIndex = sheetReport.Charts.Add(ChartType.Bar3DClustered, 8, 0, 32, 7);
            leadSourceColumn = sheetReport.Charts[chartIndex];

            //Chart title
            leadSourceColumn.Title.Text = string.Format("Doanh số dịch vụ chi quầy từng thị trường\n {0} \n Giai đoạn: {1}", namePartner, text);
            leadSourceColumn.Title.Font.Color = Color.Silver;

            // Get dữ liệu của list data
            List<ReportDetailtForPartner> listDataItem = listReportData.Where(x => x.Type == 1).ToList();

            string[] listTotalRowData = new string[listDataItem.Count];

            int i = 0;
            string categoryData = string.Empty;
            foreach (ReportDetailtForPartner item in listDataItem)
            {

                listTotalRowData[i] = string.Concat("{"
                    , string.Format("{0}, {1}, {2}"
                    , item.DSChiQuay, item.DSChiNha, item.DSCK
                    )
                    , "}");

                leadSourceColumn.NSeries.Add(listTotalRowData[i], true);

                categoryData = "{Doanh số Chi Quầy, Doanh số Chi Nhà, Doanh số Chuyển Khoản}";
                leadSourceColumn.NSeries.CategoryData = categoryData;

                leadSourceColumn.NSeries[i].Name = string.Format("Lũy kế {0} {1}", textValue, item.Year);
                i++;
            }
            ...
            List<ReportDetailtForPartner> listDataGradation = new ReportBL().ReportDetailtPartnerGradationCompareForOne(year, int.Parse(gradationID), reportTypeID, partnerID);
        ...
        {
            WorkbookDesigner designer = new WorkbookDesigner();
            string templatePath = "~/Content/Report/ReportDetailtByPartnerLHForCompare.xlsx";
            // Get đường dẫn
            templatePath = System.Web.HttpContext.Current.Server.MapPath(templatePath);
            ...
            // Tổng số row theo table1
            int totalRowTable1 = table.Rows.Count + 6;
            ...
            listDataCompareMonth = new ReportBL().ReportDetailtPartnerCompareMonthForAll(year, month, reportTypeID);
            ...
            string title = "Bảng số liệu Tăng giảm so với cùng kì theo từng thị trường (USD)";
        ...
        {
            WorkbookDesigner designer = new WorkbookDesigner();
            string templatePath = "~/Content/Report/ReportDetailtByPartnerLHCompareForOne.xlsx";
            // Get đường dẫn
            templatePath = System.Web.HttpContext.Current.Server.MapPath(templatePath);
            ...
            {
                namePartner = listDataCompareMonth[0].PartnerName;
            }
            ...
            //// Adding SeriesCollection (chart data source) to the chart ranging from "A1" cell to "B3"
            //string totalRowData = "C22:E24";
            //leadSourceColumn.NSeries.Add(totalRowData, true);

            //// Set the category data covering the range A2:A5.
            //// Tổng số dòng cần hiển thị là số dòng hiện tại  + số dòng trong listData -2 (trong đó 1 là dòng tổng cuối cùng)
            //string categoryData = "C21:E21";
            //leadSourceColumn.NSeries.CategoryData = categoryData;
            ...
            listDataCompareMonth = new ReportBL().ReportDetailtPartnerCompareMonthForOne(year, month, reportTypeID, partnerID);
            ...
            // Vẻ biểu đồ tròn
            // Month
            ReportDetailtForPartner dataPieMonth = listDataCompareMonthConvert.Find(x => x.Month == month.ToString() && x.Year == year.ToString());

            // last month
            ReportDetailtForPartner dataPieLastMonth = listDataCompareMonthConvert.Find(x => x.Month == (month - 1).ToString() && x.Year == year.ToString());
            if (month == 1)
            {
            ...
            if (dataPieMonth != null)
            {
                ...
                leadSourcePie.Elevation = 45;
                // Set properties of chart title
                leadSourcePie.Title.Text = string.Format("Tỉ trọng các đối tác theo thị trường Tháng {0}/{1} \n {2}", month, year, namePartner);
            ...
            if (dataPieLastMonth != null)
            {
                ...
                if (month == 1)
                {
                    leadSourcePie.Title.Text = string.Format("Tỉ trọng các đối tác theo thị trường Tháng {0}/{1} \n {2}", 12, year - 1, namePartner);
                }
                else
                {

                    leadSourcePie.Title.Text = string.Format("Tỉ trọng các đối tác theo thị trường Tháng {0}/{1} \n {2}", month - 1, year, namePartner);
                }
                leadSourcePie.Title.Font.Color = Color.Silver;
                leadSourcePie.Title.Font.IsBold = true;
                leadSourcePie.Title.Font.Size = 12;
        ...
        /// </summary>
        /// <returns></returns>
        private DataTable CreateDataTableFormart(bool checkForOne = true)
        {
        ...
        private void FillData(DataTable mother, DataTable fill, bool checkForOne = true)
        {
            int stt = 1;
